...
export const ReportModal: React.FC<Props> = ({ encounter, onClose }) => {
  ...
  const handleExportHtml = async () => {
    ...
    // Aggregate crit data for Graph 2
        ...
        <div class="container">
          ...
          <div class="summary">
            <div class="summary-item">
              ...
              <span class="summary-val">${durationSec.toFixed(1)}s</span>
            </div>
            <div class="summary-item">
              <span class="summary-label">Total Dealt</span>
              <span class="summary-val">${encounter.totalDamageDealt.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
            </div>
            <div class="summary-item">
              <span class="summary-label">Total Received</span>
              <span class="summary-val">${encounter.totalDamageReceived.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
            </div>
          </div>
    ...
    const addTable = (title: string, skills: Record<string, SkillDamageRow>, extraHtml: string = "") => {
      ...
      sorted.forEach(row => {
        ...
        html += `<tr>
          ...
          <td class="numeric" style="color: #aaa;">${row.critHits} <span style="font-size: 0.8em; color: #666;">(${critPercent}%)</span></td>
          <td class="numeric" style="font-weight: bold;">${row.totalDamage.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
          <td class="numeric" style="color: #ddd;">${row.dps.toLocaleString(undefined, { maximumFractionDigits: 1 })}</td>
    ...
    Object.entries(damageByType).sort((a, b) => b[1] - a[1]).forEach(([type, val]) => {
      ...
      html += `
        <div class="graph-row">
          <div class="graph-label ${getColorClass(type)}">${type}</div>
          <div class="graph-bar-container">
            <div class="graph-bar" style="width: ${pct}%; background: ${hex};"></div>
          </div>
          <div class="graph-val">${val.toLocaleString(undefined, { maximumFractionDigits: 0 })} (${overallPct}%)</div>
        </div>
      `;
    });
    ...
    dealtSorted.forEach(row => {
      ...
      html += `
        <div class="graph-row">
          ...
          <div class="graph-val">${totalAilment.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
        </div>
        <div style="font-size: 0.8em; color: #888; margin-left: 265px; margin-bottom: 20px; margin-top: -10px;">
          ${bleedDmg > 0 ? `<span style="color:var(--color-physical)">Bleed: ${bleedDmg.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span> ` : ''}
          ${igniteDmg > 0 ? `<span style="color:var(--color-fire)">Ignite: ${igniteDmg.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>` : ''}
        </div>
      `;
    });
    html += `</div>`; // End Tab 4
                    ...
                    .map(([type, val]) => {
                      return (
                        <div key={type} style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span>{type}</span>
                          <span>
                            {val.toLocaleString(undefined, { maximumFractionDigits: 0 })} ({((val / row.totalDamage) * 100).toFixed(1)}%)
                          </span>
                        </div>
                      );
                    })}
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <div style={{ marginBottom: 15, fontSize: 14, color: '#ccc' }}>
          <strong>Duration:</strong> {durationSec.toFixed(1)}s
          <br/>
          <strong>Total Dealt:</strong> {encounter.totalDamageDealt.toLocaleString(undefined, { maximumFractionDigits: 0 })}
          <br/>
